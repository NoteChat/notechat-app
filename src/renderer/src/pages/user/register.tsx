  ...
  const sendValidateCode = (evt) => {
    ...
    if (!loading) {
      ...
      if (email) {
        setLoading(true)
        API.v1
          .getValidCode({ email: email.value })
          .then((res: any) => {
            const { data } = res
            if (data.code === 1000) {
    ...
    <LanguageLayout>
      <div className="centerBlock" style={{ top: '40%' }}>
        ...
        <Form.Root className="FormRoot m-auto mt-5" onSubmit={onSubmit} method="POST">
          ...
          {/* <Form.Field className="FormField" name="acceptPolicy">
            <div
              style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}
            >
              <Form.Label className="FormLabel">{t('acceptPolicy.check.required')}</Form.Label>
              <Form.Message className="FormMessage" match="valueMissing">
                {t('acceptPolicy.check.required')}
              </Form.Message>
            </div>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <label className="Label" htmlFor="c1">
                <Link
                  to={`https://notechat.xyz/${lang === 'en' ? lang + '/' : ''}policy`}
                  className="color-blue cursor-pointer ml-2"
                  >
                  《{t('policy.label')}》
                </Link>
